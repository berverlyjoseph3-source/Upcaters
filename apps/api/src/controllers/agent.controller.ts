// enterprise-ai-agent-platform/apps/api/src/controllers/agent.controller.ts
...
export class AgentController {
   ...
   */
  static async execute(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      logger.error({ error, userId: req.user?.id }, 'Agent execution failed');
      res.status(500).json({
        success: false,
   ...
   */
  static async executeStream(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response = await AgentService.executeAgentStream(
        req.user.id,
        input,
        sessionId || `session_${req.user.id}_${Date.now()}`,
        agentType || 'orchestrator',
      ...
      logger.error({ error, userId: req.user?.id }, 'Agent streaming failed');
      res.write(`data: ${JSON.stringify({
        type: 'error',
        error: error instanceof Error ? error.message : 'Agent execution failed',
        timestamp: new Date().toISOString(),
   ...
   */
  static async getAgentStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (errorCount === totalCount) overallStatus = 'down';
      else if (errorCount > 0) overallStatus = 'degraded';
   ...
   */
  static async listAgents(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
   ...
   */
  static async getAgentTools(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      logger.error({ error, agentType: req.params.agentType }, 'Failed to get agent tools');
      res.status(500).json({
        success: false,
   ...
   */
  static async getAgentHealth(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { agentType } = req.params;
      const agent = agentRegistry.getAgent(agentType as AgentType);
      ...
      logger.error({ error, agentType: req.params.agentType }, 'Failed to get agent health');
      res.status(500).json({
        success: false,
   ...
   */
  static async getAgentMetrics(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { agentType } = req.params;
      const agent = agentRegistry.getAgent(agentType as AgentType);
      ...
      logger.error({ error, agentType: req.params.agentType }, 'Failed to get agent metrics');
      res.status(500).json({
        success: false,
   ...
   */
  static async executeSpecificAgent(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { agentType } = req.params;
      ...
      const response = await AgentService.executeSpecificAgent(
        req.user.id,
      ...
      logger.error({ error, userId: req.user?.id, agentType: req.params.agentType }, 'Agent execution failed');
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Agent execution failed',
        code: 'EXECUTION_ERROR',
  ...
  static async generateImage(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response = await AgentService.executeSpecificAgent(
        req.user.id,
        AgentType.CONTENT,
        `Generate image: ${prompt}${size ? ` Size: ${size}` : ''}${quality ? ` Quality: ${quality}` : ''}`,
        `session_${req.user.id}_${Date.now()}`
      );
      ...
      logger.error({ error, userId: req.user?.id }, 'Image generation failed');
      res.status(500).json({
        success: false,
  ...
  static async generateVideo(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response = await AgentService.executeSpecificAgent(
        req.user.id,
        AgentType.CONTENT,
        `Generate video: ${prompt}${duration ? ` Duration: ${duration} seconds` : ''}${style ? ` Style: ${style}` : ''}`,
        `session_${req.user.id}_${Date.now()}`
      );
      ...
      logger.error({ error, userId: req.user?.id }, 'Video generation failed');
      res.status(500).json({
        success: false,
   ...
   */
  static async multiPlatformPost(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response = await AgentService.executeSpecificAgent(
        req.user.id,
        AgentType.SOCIAL,
        `Post to ${platformsList.join(', ')}: ${content}${mediaUrl ? ` with image ${mediaUrl}` : ''}`,
        `session_${req.user.id}_${Date.now()}`
      );
      ...
      logger.error({ error, userId: req.user?.id }, 'Multi-platform post failed');
      res.status(500).json({
        success: false,
  ...
  static async startSession(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const session = await AgentService.createSession(req.user.id, sessionId);
      ...
      logger.error({ error, userId: req.user?.id }, 'Failed to start session');
      res.status(500).json({
        success: false,
  ...
  static async endSession(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      await AgentService.endSession(req.user.id, sessionId);
      ...
      logger.error({ error, userId: req.user?.id, sessionId: req.params.sessionId }, 'Failed to end session');
      res.status(500).json({
        success: false,
  ...
  static async getSession(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const session = await AgentService.getSession(req.user.id, sessionId);
      ...
      logger.error({ error, userId: req.user?.id, sessionId: req.params.sessionId }, 'Failed to get session');
      res.status(500).json({
        success: false,
   ...
   */
  static async resetMetrics(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
   ...
   */
  static async resetAgent(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (req.user?.role !== 'ADMIN') {
        res.status(403).json({
          success: false,
      ...
      logger.error({ error, agentType: req.params.agentType }, 'Failed to reset agent');
      res.status(500).json({
        success: false,
