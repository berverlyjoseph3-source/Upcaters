...
import { PLAN_LIMITS_CONFIG, FEATURE_ACCESS_MATRIX, ACTION_COSTS } from '../types/usage.types';
 ...
 */
router.get('/', RateLimitMiddleware.relaxed(), (req, res) => {
  res.json({
 ...
 */
router.get('/plans', RateLimitMiddleware.relaxed(), (req, res) => {
  const plans = Object.entries(PLAN_LIMITS_CONFIG).map(([id, config]) => ({
    id,
    name: id === 'FREE' ? 'Free' : id === 'STARTER' ? 'Starter' : id === 'PROFESSIONAL' ? 'Professional' : 'Enterprise',
    limits: config,
    features: FEATURE_ACCESS_MATRIX[id] || FEATURE_ACCESS_MATRIX.FREE,
    pricing: {
 ...
 */
router.get('/actions', RateLimitMiddleware.relaxed(), (req, res) => {
  const actions = Object.entries(ACTION_COSTS).map(([type, config]) => ({
    actionType: type,
 ...
 */
router.get('/agents', RateLimitMiddleware.relaxed(), (req, res) => {
  const agents = [
    {
      type: 'email',
      name: 'Email Agent',
 ...
 */
router.get('/webhooks', RateLimitMiddleware.relaxed(), (req, res) => {
  res.json({
    success: true,
    data: {
      stripe: {
 ...
 */
router.get('/errors', RateLimitMiddleware.relaxed(), (req, res) => {
  res.json({
    success: true,
    data: {
      auth: [
