...
export class WebAgent extends BaseAgent {
  ...
  constructor() {
    super(
      AgentType.WEB,
      'Web Agent',
      'Web search, research, weather, and data extraction',
      '1.0.0'
    );
  ...
  protected registerTools(): void {
    this.registerTool(WebTools.searchWebTool());
    this.registerTool(WebTools.getWeatherTool());
    this.registerTool(WebTools.researchTool());
    this.registerTool(WebTools.getNewsTool());
    this.registerTool(WebTools.getAirQualityTool());
    this.registerTool(WebTools.compareWeatherTool());
    this.registerTool(WebTools.searchImagesTool());
  }
  ...
  protected async doExecute(request: AgentRequest, context: AgentContext): Promise<any> {
    ...
    try {
      ...
      logger.error({ error, userId: context.userId, executionTimeMs: Date.now() - startTime }, 'Web agent execution failed');
  ...
  private isImageSearchRequest(input: string): boolean {
   ...
   */
  private async handleWeather(input: string): Promise<any> {
    try {
      ...
      const result = await WebTools.getWeather(location, Math.min(days, 7), units);
      ...
      logger.error({ error, input }, 'Weather handling failed');
   ...
   */
  private async handleResearch(input: string): Promise<any> {
    try {
      let query = input;
      ...
      const result = await WebTools.research(query, 'detailed');
      ...
      logger.error({ error, input }, 'Research handling failed');
   ...
   * Handle news request
   */
  private async handleNews(input: string): Promise<any> {
    try {
      let topic = input;
      ...
      const result = await WebTools.getNews(topic, 10);
      ...
      logger.error({ error, input }, 'News handling failed');
   ...
   */
  private async handleAirQuality(input: string): Promise<any> {
    try {
      const location = this.extractLocation(input);
      ...
      const result = await WebTools.getAirQuality(location);
      ...
      logger.error({ error, input }, 'Air quality handling failed');
   ...
   * Handle weather comparison
   */
  private async handleWeatherComparison(input: string): Promise<any> {
    try {
      // Extract locations
      ...
      const result = await WebTools.compareWeather(locations);
      ...
      logger.error({ error, input }, 'Weather comparison handling failed');
   ...
   * Handle image search
   */
  private async handleImageSearch(input: string): Promise<any> {
    try {
      let query = input;
      ...
      const result = await WebTools.searchImages(query, 10);
      ...
      logger.error({ error, input }, 'Image search handling failed');
   ...
   * Handle web search
   */
  private async handleWebSearch(input: string): Promise<any> {
    try {
      ...
      const result = await WebTools.searchWeb(query, 10);
      ...
      logger.error({ error, input }, 'Web search handling failed');
