...
export class PerplexityClient {
  ...
  private initializeClient(): void {
    this.client = axios.create({
      baseURL: apiConfig.perplexity.apiUrl,
      headers: {
        'Authorization': `Bearer ${apiConfig.perplexity.apiKey}`,
        'Content-Type': 'application/json',
      },
      timeout: apiConfig.timeouts.ai,
    });

    this.client.interceptors.request.use(
      (config) => {
        logger.debug({ method: config.method, url: config.url }, 'Perplexity API request');
        return config;
      },
    ...
    this.client.interceptors.response.use(
      (response) => {
        logger.debug({ status: response.status, model: response.data?.model, tokens: response.data?.usage?.total_tokens }, 'Perplexity API response');
        return response;
      },
      async (error: AxiosError) => {
        if (error.response?.status === 401) {
          ...
          logger.error('Perplexity API payment required');
        }
        throw error;
      }
    );
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < this.MAX_RETRIES) {
          ...
          logger.warn({ attempt, delay, context, error: lastError.message }, 'Perplexity API retry');
  ...
  async complete(options: PerplexityCompletionOptions): Promise<PerplexityCompletionResponse> {
  ...
  async research(query: string, detail: string = 'detailed'): Promise<PerplexityResearchResult> {
  ...
  async searchWithSources(query: string): Promise<{ answer: string; sources: string[] }> {
  ...
  async researchWithImages(query: string): Promise<{ answer: string; sources: string[]; images?: string[] }> {
    const response = await this.complete({
      query,
      returnImages: true,
      maxTokens: 3000,
  ...
  async compareTopics(topics: string[]): Promise<Array<{ topic: string; summary: string; sources?: string[] }>> {
  ...
  async analyzeSentiment(text: string): Promise<{ sentiment: 'positive' | 'negative' | 'neutral'; confidence: number; explanation: string }> {
    ...
    try {
      ...
      logger.warn({ error }, 'Failed to parse sentiment analysis response');
    }
  ...
  async factCheck(claim: string): Promise<{ claim: string; verdict: string; confidence: number; explanation: string; sources: string[] }> {
    ...
    try {
      ...
      logger.warn({ error }, 'Failed to parse fact check response');
    }
  ...
  async getLatestNews(topic: string): Promise<{ answer: string; sources: string[] }> {
  ...
  async extractKeyPoints(text: string): Promise<Array<{ point: string; importance: number }>> {
    ...
    try {
      ...
      logger.warn({ error }, 'Failed to parse key points');
    }
