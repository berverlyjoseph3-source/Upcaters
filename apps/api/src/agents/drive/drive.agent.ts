...
export class DriveAgent extends BaseAgent {
  constructor() {
    super(
      AgentType.DRIVE,
      'Drive Agent',
      'File management, search, sharing, and organization across Google Drive',
      '1.0.0'
    );
  }

  protected registerTools(): void {
    this.registerTool(DriveTools.listFilesTool());
    this.registerTool(DriveTools.searchFilesTool());
    this.registerTool(DriveTools.uploadFileTool());
    this.registerTool(DriveTools.downloadFileTool());
    this.registerTool(DriveTools.deleteFileTool());
    this.registerTool(DriveTools.trashFileTool());
    this.registerTool(DriveTools.restoreFileTool());
    this.registerTool(DriveTools.shareFileTool());
    this.registerTool(DriveTools.getFileInfoTool());
    this.registerTool(DriveTools.createFolderTool());
    this.registerTool(DriveTools.moveFileTool());
    this.registerTool(DriveTools.copyFileTool());
    this.registerTool(DriveTools.renameFileTool());
    this.registerTool(DriveTools.getQuotaInfoTool());
    this.registerTool(DriveTools.batchDeleteTool());
  }

  /**
   * Get Drive client for a user
   */
  private async getDriveClient(userId: string): Promise<DriveClient> {
  ...
  protected async doExecute(request: AgentRequest, context: AgentContext): Promise<any> {
    ...
    try {
      ...
      logger.error({ error, userId: context.userId, executionTimeMs: Date.now() - startTime }, 'Drive agent execution failed');
   ...
   * Handle list files
   */
  private async handleListFiles(userId: string, input: string): Promise<any> {
    try {
      ...
      const result = await DriveTools.listFiles(userId, {
        pageSize,
        q: query,
        orderBy,
      });
      ...
      logger.error({ error, userId }, 'List files handling failed');
      return {
        success: false,
   ...
   * Handle upload file
   */
  private async handleUpload(userId: string, input: string): Promise<any> {
    try {
      ...
      const result = await DriveTools.uploadFile(userId, {
        name,
      ...
      logger.error({ error, userId }, 'Upload handling failed');
      return {
        success: false,
   ...
   */
  private async handleDownload(userId: string, input: string): Promise<any> {
    try {
      const fileIdMatch = input.match(/(?:id|file)[:\s]+([a-zA-Z0-9_-]+)/i);
      
      if (!fileIdMatch) {
        // Show recent files to choose from
        const files = await DriveTools.listFiles(userId, { pageSize: 5 });
      ...
      const result = await DriveTools.downloadFile(userId, fileIdMatch[1]);
      ...
      logger.error({ error, userId }, 'Download handling failed');
      return {
        success: false,
   ...
   * Handle share file
   */
  private async handleShare(userId: string, input: string): Promise<any> {
    try {
      const fileIdMatch = input.match(/(?:id|file)[:\s]+([a-zA-Z0-9_-]+)/i);
      ...
      const result = await DriveTools.shareFile(userId, {
        fileId: fileIdMatch[1],
        email: emailMatch[1],
        role: roleMatch?.[1] as any || 'reader',
      ...
      logger.error({ error, userId }, 'Share handling failed');
      return {
        success: false,
   ...
   * Handle create folder
   */
  private async handleCreateFolder(userId: string, input: string): Promise<any> {
    try {
      ...
      const result = await DriveTools.createFolder(userId, name, parentMatch?.[1]);
      ...
      logger.error({ error, userId }, 'Create folder handling failed');
      return {
        success: false,
   ...
   * Handle move file
   */
  private async handleMoveFile(userId: string, input: string): Promise<any> {
    try {
      ...
      const result = await DriveTools.moveFile(userId, fileIdMatch[1], folderIdMatch[1]);
      ...
      logger.error({ error, userId }, 'Move file handling failed');
      return {
        success: false,
   ...
   * Handle rename file
   */
  private async handleRename(userId: string, input: string): Promise<any> {
    try {
      const fileIdMatch = input.match(/(?:file|id)[:\s]+([a-zA-Z0-9_-]+)/i);
      ...
      const result = await DriveTools.renameFile(userId, fileIdMatch[1], nameMatch[1]);
      ...
      logger.error({ error, userId }, 'Rename handling failed');
      return {
        success: false,
   ...
   * Handle copy file
   */
  private async handleCopyFile(userId: string, input: string): Promise<any> {
    try {
      const fileIdMatch = input.match(/(?:file|id)[:\s]+([a-zA-Z0-9_-]+)/i);
      const nameMatch = input.match(/(?:name|as|called)\s+["']?([^"'\n]+)["']?/i);

      const result = await DriveTools.copyFile(userId, fileIdMatch![1], nameMatch?.[1]);
      ...
      logger.error({ error, userId }, 'Copy file handling failed');
      return {
        success: false,
   ...
   * Handle delete file
   */
  private async handleDelete(userId: string, input: string): Promise<any> {
    try {
      ...
      const result = await DriveTools.batchDelete(userId, fileIds);
      ...
      logger.error({ error, userId }, 'Delete handling failed');
      return {
        success: false,
   ...
   * Handle restore file
   */
  private async handleRestore(userId: string, input: string): Promise<any> {
    try {
      const fileIdMatch = input.match(/(?:file|id)[:\s]+([a-zA-Z0-9_-]+)/i);
      ...
      await DriveTools.restoreFile(userId, fileIdMatch[1]);
      ...
      logger.error({ error, userId }, 'Restore handling failed');
      return {
        success: false,
   ...
   * Handle search files
   */
  private async handleSearch(userId: string, input: string): Promise<any> {
    try {
      let query = input;
      ...
      const result = await DriveTools.searchFiles(userId, {
        query: query.trim(),
        pageSize: 20,
      });
      ...
      logger.error({ error, userId }, 'Search handling failed');
      return {
        success: false,
   ...
   * Handle quota info
   */
  private async handleQuotaInfo(userId: string, input: string): Promise<any> {
    try {
      const result = await DriveTools.getQuotaInfo(userId);
      ...
      logger.error({ error, userId }, 'Quota info handling failed');
      return {
        success: false,
   ...
   * Handle file info
   */
  private async handleFileInfo(userId: string, input: string): Promise<any> {
    try {
      const fileIdMatch = input.match(/(?:file|id)[:\s]+([a-zA-Z0-9_-]+)/i);
      ...
      const result = await DriveTools.getFileInfo(userId, fileIdMatch[1]);
      ...
      logger.error({ error, userId }, 'File info handling failed');
      return {
        success: false,
