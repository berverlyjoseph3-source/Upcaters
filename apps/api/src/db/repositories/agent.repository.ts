...
import { Prisma, AgentType, ExecutionStatus } from '@prisma/client';
...
export interface AgentExecutionInput {
  ...
  ipAddress?: string;
  userAgent?: string;
  /** ENHANCEMENT: Execution ID for state persistence */
  executionId?: string;
  /** ENHANCEMENT: Plan ID for state persistence */
  planId?: string;
}
...
export interface AgentExecutionUpdate {
  ...
  costUsd?: number;
  completedAt?: Date;
  /** ENHANCEMENT: Step status for execution state */
  stepStatus?: StepTaskState['status'];
  /** ENHANCEMENT: Retry count */
  retryCount?: number;
  /** ENHANCEMENT: Whether fallback was used */
  fallbackUsed?: boolean;
  /** ENHANCEMENT: Fallback agent type */
...
export interface PaginationParams {
  page?: number;
  limit?: number;
  sortBy?: 'createdAt' | 'durationMs' | 'tokensUsed' | 'costUsd';
  sortOrder?: 'asc' | 'desc';
}
...
export class AgentRepository {
  ...
  static async createExecution(
    ...
    try {
      ...
      logger.error({ error, data }, 'Failed to create agent execution');
      throw error;
    }
  }
  ...
  static async updateExecution(
    ...
    try {
      ...
      logger.debug({ executionId, ...data }, 'Agent execution updated');
    } catch (error) {
      logger.error(
        { error, executionId, data },
   ...
   */
  static async getUserExecutions(
    userId: string,
    ...
    try {
      ...
      const where: Prisma.AgentExecutionWhereInput = { userId };
      ...
      const orderBy: Prisma.AgentExecutionOrderByWithRelationInput = {
        [sortBy]: sortOrder,
      };
  ...
  static async getExecutionStats(
    ...
    try {
      ...
      // Group by status
      const byStatus = await prisma.agentExecution.groupBy({
        by: ['status'],
        where,
        _count: { id: true },
        _sum: { costUsd: true },
      ...
      // Group by agent type
      const byAgentType = await prisma.agentExecution.groupBy({
        by: ['agentType'],
        where,
        _count: { id: true },
      ...
      const byActionType = await prisma.agentExecution.groupBy({
        by: ['actionType'],
        where,
        _count: { id: true },
        _sum: { costUsd: true },
      ...
      const recentDurations = await prisma.agentExecution.findMany({
      ...
      return {
        ...
        byAgentType: byAgentType.reduce(
          (acc, curr) => {
            acc[curr.agentType] = {
   ...
   */
  static async getFailedExecutionsForRetry(
    maxRetries: number = 3,
    limit: number = 100,
    ...
    try {
      ...
      logger.error(
        { error },
        'Failed to get failed executions for retry',
      );
      throw error;
    }
  }
  ...
  static async getExecutionTimeline(
    ...
    try {
      ...
      for (const exec of executions) {
        ...
        } else if (exec.status === ExecutionStatus.ERROR) {
      ...
      logger.error(
        { error, userId },
        'Failed to get execution timeline',
      );
      throw error;
    }
  }
  ...
  static async saveExecutionState(
    ...
    try {
      ...
      logger.error(
        { error, executionId: state.executionId },
   ...
   */
  static async loadExecutionState(
    executionId: string,
    ...
    try {
      ...
      if (!execution || !execution.metadata) {
        logger.warn(
          { executionId },
          'No persisted execution state found',
        );
        return null;
      }
      ...
      const state: ExecutionTaskState = {
        executionId: metadata.executionId || executionId,
        planId: metadata.planId || '',
        stepStates: (metadata.stepStates || []).map(
          (step: any) => ({
            stepId: step.stepId,
      ...
      logger.info(
        {
          executionId,
          overallStatus: state.overallStatus,
          completedSteps: state.stepStates.filter(
            (s) => s.status === 'completed',
          ).length,
          failedSteps: state.stepStates.filter(
            (s) => s.status === 'failed',
          ).length,
        },
   ...
   */
  static async deleteExecutionState(
    executionId: string,
  ): Promise<void> {
    try {
   ...
   */
  static async listExecutionStates(
    status?: ExecutionTaskState['overallStatus'],
    limit: number = 50,
    offset: number = 0,
  ): Promise<{ states: ExecutionTaskState[]; total: number }> {
    try {
      const where: Prisma.AgentExecutionWhereInput = {
        actionType: 'execution_state',
      };
      ...
      const states: ExecutionTaskState[] = [];
   ...
   */
  static async cleanupOldExecutionStates(
    daysToKeep: number = 7,
  ): Promise<number> {
    try {
      ...
      logger.info(
        { deletedCount: result.count, daysToKeep },
        'Old execution states cleaned up',
      );
      ...
      logger.error(
        { error },
        'Failed to clean up old execution states',
      );
      throw error;
    }
  }
   ...
   */
  static async getAgentErrorRate(
    agentType: AgentType,
    windowMs: number = 60000,
  ): Promise<{ total: number; failed: number; errorRate: number }> {
    try {
      ...
      logger.error(
        { error, agentType },
        'Failed to get agent error rate',
      );
   ...
   */
  static async getRecentAgentErrors(
    agentType: AgentType,
    limit: number = 10,
  ) {
    try {
      ...
      logger.error(
        { error, agentType },
        'Failed to get recent agent errors',
      );
      return [];
    }
  }
   ...
   */
  static async getExecutionCostBreakdown(
    executionId: string,
  ): Promise<
    Array<{
      agentType: string;
  ...
  > {
    try {
      const breakdown = await prisma.agentExecution.groupBy({
        by: ['agentType'],
        where: {
          metadata: {
            path: ['executionId'],
      ...
      logger.error(
        { error, executionId },
        'Failed to get execution cost breakdown',
      );
      return [];
    }
  }
   ...
   */
  static async bulkCreateExecutions(
    ...
    try {
      ...
      logger.info(
        { count: result.count },
        'Bulk executions created',
      );

      return result.count;
    } catch (error) {
      logger.error({ error }, 'Failed to bulk create executions');
      throw error;
    }
  }
  ...
  static async cleanupOldExecutions(
    ...
    try {
      ...
      logger.error(
        { error },
        'Failed to clean up old executions',
      );
      throw error;
    }
  }
