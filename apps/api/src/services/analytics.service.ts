...
export interface UsageSummary {
  aiActions: { current: number; previous: number; change: number; changePercentage: number; trend: 'up' | 'down' | 'stable' };
  apiCalls: { current: number; previous: number; change: number; changePercentage: number; trend: 'up' | 'down' | 'stable' };
  totalCost: { current: number; previous: number; change: number; changePercentage: number; trend: 'up' | 'down' | 'stable' };
  totalTokens: { current: number; previous: number; change: number; changePercentage: number; trend: 'up' | 'down' | 'stable' };
  activeDays: number;
...
export class AnalyticsService {
  ...
  static async getAnalytics(
    ...
    const effectiveStart = startDate || subDays(new Date(), 30);
  ...
  static async getUsageSummary(
    ...
    // Count active days
    const activeDays = await prisma.agentExecution.groupBy({
      by: ['createdAt'],
      where: {
        userId,
   ...
   */
  static async getDailyUsage(userId: string, startDate: Date, endDate: Date): Promise<DailyUsage[]> {
    ...
    for (const exec of executions) {
      const date = format(exec.createdAt, 'yyyy-MM-dd');
   ...
   */
  static async getUsageByAgent(userId: string, startDate: Date, endDate: Date): Promise<UsageByAgent[]> {
   ...
   */
  static async getUsageByAction(
    userId: string,
    startDate: Date,
    endDate: Date
  ): Promise<Array<{ actionType: string; count: number; cost: number; category: string }>> {
   ...
   * Get cost breakdown
   */
  static async getCostBreakdown(userId: string, startDate: Date, endDate: Date): Promise<any> {
   ...
   * Get usage forecast
   */
  static async getForecast(userId: string, startDate: Date, endDate: Date, days: number = 30): Promise<any[]> {
   ...
   * Get top actions
   */
  static async getTopActions(
    userId: string,
    startDate: Date,
    endDate: Date,
    limit: number = 10
  ): Promise<Array<{ actionType: string; count: number; cost: number }>> {
    const usageLogs = await prisma.usageLog.findMany({
      where: {
        userId,
        updatedAt: { gte: startDate, lte: endDate },
  ...
  static async exportAnalytics(
    ...
    if (format === 'json') {
      ...
      const rows = analytics.dailyUsage.map((d: DailyUsage) => [
        d.date,
        d.aiActions,
        d.apiCalls,
        d.cost,
   ...
   */
  static async getPlatformAnalytics(startDate?: Date, endDate?: Date): Promise<any> {
    const effectiveStart = startDate || subMonths(new Date(), 1);
    const effectiveEnd = endDate || new Date();

    const [totalUsers, activeUsers, totalExecutions, totalCost, usersByPlan] = await Promise.all([
      prisma.user.count(),
      prisma.user.count({ where: { lastLoginAt: { gte: subDays(new Date(), 7) } } }),
  ...
  static async getRevenueAnalytics(
    ...
    const effectiveStart = startDate || subYears(new Date(), 1);
    const effectiveEnd = endDate || new Date();

    const revenue = await prisma.billingInvoice.findMany({
      where: {
        status: 'paid',
    ...
    let groupedRevenue: Record<string, number> = {};
    for (const inv of revenue) {
      let key: string;
      if (period === 'day') {
        key = format(inv.createdAt, 'yyyy-MM-dd');
      } else if (period === 'week') {
        key = format(inv.createdAt, 'yyyy-ww');
      } else if (period === 'month') {
        key = format(inv.createdAt, 'yyyy-MM');
      } else {
        key = format(inv.createdAt, 'yyyy');
      }
