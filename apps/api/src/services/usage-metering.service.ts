...
export class PlanLimitExceededError extends Error {
  constructor(
    public category: 'ai_action' | 'api_call',
    public used: number,
    public limit: number,
    public resetDate: Date,
    public overageCost: number,
    public upgradeUrl: string,
  ) {
    super(
      `${category} limit exceeded: ${used}/${limit} (overage cost: $${overageCost.toFixed(4)})`,
    );
...
export class RateLimitWarning extends Error {
  constructor(
    message: string,
...
export interface UsageAlertThresholds {
  ...
  critical: number; // Percentage (default 95)
  overLimit: boolean;
}
...
export class UsageMeteringService {
  ...
  private static readonly OVERAGE_KEY_PREFIX = 'overage:';
  ...
  private static failedIncrements: Array<{
    userId: string;
    actionType: ActionType;
    cost: number;
    tokensUsed: number;
  ...
  static initRedis(redisUrl: string): void {
    if (!this.redis) {
      ...
      this.redis.on('error', (error) => {
        logger.error({ error }, 'Usage metering Redis error');
      });

      this.redis.on('connect', () => {
        logger.info('Usage metering Redis connected');
        // Retry any failed increments
        this.retryFailedIncrements();
      });

      logger.info('Usage metering Redis client initialized');
    }
  }
  ...
  private static getImageOverageKey(
    userId: string,
    billingPeriod: string,
  ): string {
  ...
  private static getVideoOverageKey(
    userId: string,
    billingPeriod: string,
  ): string {
  ...
  private static async getUserPlanLimits(userId: string): Promise<{
    planId: string;
    aiActions: number;
    apiCalls: number;
    overagePricing: {
    ...
    const limits =
      PLAN_LIMITS_CONFIG[user.planId] || PLAN_LIMITS_CONFIG.FREE;
    const overagePricing =
      OVERAGE_PRICING_CONFIG[user.planId] ||
      OVERAGE_PRICING_CONFIG.FREE;

    return {
      planId: user.planId,
      aiActions: limits.aiActions as number,
      apiCalls: limits.apiCalls as number,
      overagePricing,
    };
  }
  ...
  static async incrementUsage(
    ...
    try {
      const actionCost = ACTION_COSTS[actionType];
      if (!actionCost) {
        logger.warn(
          { userId, actionType },
          'Unknown action type for usage metering',
        );
        return {
          success: false,
          actionType,
      ...
      if (this.isRedisAvailable()) {
        return await this.redisIncrementUsage(
          userId,
          actionType,
          actionCost,
          cost,
      ...
      return await this.dbIncrementUsage(
        userId,
        actionType,
        actionCost,
        cost,
   ...
   */
  private static async redisIncrementUsage(
    userId: string,
    actionType: ActionType,
    actionCost: { category: 'ai_action' | 'api_call'; baseCost: number; tokenMultiplier?: number },
    cost: number,
    tokensUsed: number,
    billingPeriod: string,
  ): Promise<UsageIncrementResult> {
    ...
    const overageKey = this.getOverageKey(
      userId,
      actionCost.category,
      billingPeriod,
    );
    ...
    // Get current usage
    const [currentActionCount, currentCategoryTotal, limits] =
      await Promise.all([
        redis.get(key),
    ...
    if (isOverage || willExceedLimit) {
      ...
      multi.incrby(
        overageKey,
        overageAmount > 0 ? overageAmount : 0,
      );
      multi.expire(overageKey, this.CACHE_TTL_SECONDS);

      // Track image/video overage separately
      if (actionType === ActionType.AI_CONTENT_IMAGE) {
        const imageKey = this.getImageOverageKey(
          userId,
          billingPeriod,
        );
        multi.incrby(imageKey, 1);
        multi.expire(imageKey, this.CACHE_TTL_SECONDS);
      } else if (actionType === ActionType.AI_CONTENT_VIDEO) {
        const videoKey = this.getVideoOverageKey(
          userId,
          billingPeriod,
        );
    ...
    this.persistToDatabase(
      ...
      logger.error(
        { error: err, userId, actionType },
        'Failed to persist usage to database',
      );
    });
   ...
   */
  private static async dbIncrementUsage(
    userId: string,
    actionType: ActionType,
    actionCost: { category: 'ai_action' | 'api_call'; baseCost: number },
    cost: number,
    tokensUsed: number,
    billingPeriod: string,
  ): Promise<UsageIncrementResult> {
   ...
   */
  private static trackFailedIncrement(
    userId: string,
    actionType: ActionType,
    cost: number,
    tokensUsed: number,
   ...
   */
  private static async retryFailedIncrements(): Promise<void> {
    if (this.failedIncrements.length === 0) return;
    ...
    logger.info(
      ...
      'Retried failed usage increments',
    );
  }
   ...
   */
  static getFailedIncrementCount(): number {
  ...
  static async checkLimit(
    ...
    try {
      ...
      if (this.isRedisAvailable()) {
        ...
        // Fallback to DB
        const dbUsage = await prisma.usageLog.aggregate({
          where: {
            userId,
            billingPeriod,
            actionType: {
      ...
      return {
        ...
        overagePrice:
          remaining <= 0 ? overagePrice : undefined,
      };
    } catch (error) {
      logger.error(
        { error, userId, actionType },
   ...
   */
  static async canAffordExecution(
    userId: string,
    ...
    try {
      ...
      const overageCost =
        aiOverage * limits.overagePricing.aiAction +
      ...
      if (overageCost > 10) {
        ...
        if (currentPlanIndex < plans.length - 1) {
          ...
          const nextPlanConfig =
            PLAN_LIMITS_CONFIG[nextPlanId];
          const nextPlanOverage =
            OVERAGE_PRICING_CONFIG[nextPlanId];

          const nextPlanAiOverage = Math.max(
            0,
            newAiActions - (nextPlanConfig.aiActions as number),
          );
          const nextPlanApiOverage = Math.max(
            0,
            newApiCalls - (nextPlanConfig.apiCalls as number),
          );
          const nextPlanOverageCost =
            nextPlanAiOverage *
      ...
      return {
        ...
        remainingAfter: {
          aiActions: Math.max(
            0,
            limits.aiActions - newAiActions,
          ),
          apiCalls: Math.max(
            0,
            limits.apiCalls - newApiCalls,
          ),
        },
        overageCost,
      ...
      logger.error(
        { error, userId },
        'Pre-execution cost check failed',
      );
      return {
        allowed: true,
        estimatedCost: 0,
  ...
  static async getUsageStats(
    ...
    try {
      ...
      if (this.isRedisAvailable()) {
        ...
        const aiActionsKey = this.getCategoryKey(
          userId,
          'ai_action',
          billingPeriod,
        );
        const apiCallsKey = this.getCategoryKey(
          userId,
          'api_call',
          billingPeriod,
        );
        const aiOverageKey = this.getOverageKey(
          userId,
          'ai_action',
          billingPeriod,
        );
        const apiOverageKey = this.getOverageKey(
          userId,
          'api_call',
          billingPeriod,
        );
        ...
        for (const key of keys) {
          ...
          if (count) {
            const actionTypeKey = key.split(':').pop() as ActionType;
            const actionCostConfig =
              ACTION_COSTS[actionTypeKey];
            if (actionCostConfig) {
        ...
        for (const log of usageLogs) {
          ...
          const agent =
            log.actionType.split('_')[1] || 'other';
          if (!byAgent[agent]) {
            byAgent[agent] = { count: 0, cost: 0 };
          }
  ...
  private static async getHistoricalUsage(
    ...
    try {
      ...
      const aiUsageLogs = await prisma.usageLog.groupBy({
        by: ['billingPeriod'],
        where: {
          userId,
          billingPeriod: { in: periods },
      ...
      const apiUsageLogs = await prisma.usageLog.groupBy({
        by: ['billingPeriod'],
        where: {
          userId,
          billingPeriod: { in: periods },
      ...
      for (const period of periods.reverse()) {
        const periodData = usageLogs.find(
          (log) => log.billingPeriod === period,
        );
      ...
      logger.error(
        { error, userId },
        'Failed to get historical usage',
      );
      return [];
    }
  }
  ...
  static async getCurrentUsage(
    userId: string,
  ): Promise<{ aiActions: number; apiCalls: number }> {
    try {
      const billingPeriod = this.getCurrentBillingPeriod();
   ...
   */
  static async getCurrentOverage(
    userId: string,
  ): Promise<OverageInfo> {
    try {
      const billingPeriod = this.getCurrentBillingPeriod();
      ...
      if (this.isRedisAvailable()) {
        ...
        const [
          ...
          redis.get(
            this.getImageOverageKey(userId, billingPeriod),
          ),
          redis.get(
            this.getVideoOverageKey(userId, billingPeriod),
          ),
        ]);
   ...
   */
  static async getUsageAlertThresholds(
    userId: string,
    ...
    return {
      ...
      overLimit:
        usage.aiActions > limits.aiActions ||
   ...
   */
  static async batchIncrementUsage(
    increments: Array<{
      userId: string;
      actionType: ActionType;
      tokensUsed?: number;
   ...
   */
  static async resetMonthlyCounters(): Promise<{
    resetCount: number;
    billingPeriod: string;
  }> {
    try {
      ...
      if (this.isRedisAvailable()) {
        ...
        logger.info(
          { billingPeriod, deletedCount: deleted },
      ...
      logger.error(
        { error },
        'Failed to reset monthly counters',
      );
      throw error;
    }
  }
   ...
   */
  static async reportStripeUsage(
    userId: string,
  ): Promise<void> {
    try {
      const user = await prisma.user.findUnique({
      ...
      const report: StripeUsageReport = {
        userId,
      ...
      const { usageReportQueue } = await import(
        '../queues/usage-report.job'
      );
   ...
   */
  static async batchReportStripeUsage(): Promise<{
    reported: number;
    failed: number;
  }> {
    ...
    for (const user of users) {
      try {
        ...
        logger.error(
          ...
          'Failed to report usage for user',
        );
        failed++;
      }
    }
    ...
    logger.info(
      { reported, failed },
      'Batch Stripe usage reporting completed',
    );
