...
// Create queues
export const analyticsReportQueue = new Queue(ANALYTICS_REPORT_QUEUE, {
...
export async function scheduleAnalyticsReport(
  ...
  await analyticsReportQueue.add(
  ...
  logger.debug({ userId, reportType, format }, 'Analytics report scheduled');
}
...
async function generateAnalyticsReport(
  ...
  const dailyBreakdown = executions.reduce((acc, e) => {
    const date = formatDate(e.createdAt, 'yyyy-MM-dd');
  ...
  if (format === 'json') {
    ...
    return {
      content,
      filename: `analytics_report_${userId}_${reportType}_${formatDate(new Date(), 'yyyy-MM-dd')}.json`,
  ...
  if (format === 'csv') {
    ...
    const rows = executions.map(e => [
      formatDate(e.createdAt, 'yyyy-MM-dd HH:mm:ss'),
      e.agentType,
      e.actionType,
      e.status,
      e.durationMs || 0,
    ...
    return {
      content,
      filename: `analytics_report_${userId}_${reportType}_${formatDate(new Date(), 'yyyy-MM-dd')}.csv`,
      mimeType: 'text/csv',
    };
  }
...
<body>
  ...
  <p><strong>Period:</strong> ${periodLabel} (${formatDate(startDate, 'MMM d, yyyy')} - ${formatDate(endDate, 'MMM d, yyyy')})</p>
  <p><strong>Generated:</strong> ${formatDate(new Date(), 'PPP p')}</p>
  ...
  <table>
    ...
    <tbody>
      ${executions.slice(0, 20).map(e => `
        <tr>
          <td>${formatDate(e.createdAt, 'MMM d, HH:mm')}</td>
          <td>${e.agentType}</td>
          <td>${e.actionType}</td>
          <td>${e.status}</td>
  ...
  return {
    content,
    filename: `analytics_report_${userId}_${reportType}_${formatDate(new Date(), 'yyyy-MM-dd')}.pdf`,
    mimeType: 'text/html',
  };
}
 ...
 */
async function processAnalyticsReport(
  userId: string,
  reportType: 'daily' | 'weekly' | 'monthly',
  format: 'csv' | 'json' | 'pdf'
): Promise<void> {
  try {
    ...
    if (!user) {
      logger.warn({ userId }, 'User not found for analytics report');
      return;
    }
    ...
    logger.info({ userId, reportType, format }, 'Analytics report sent');
  } catch (error) {
    logger.error({ error, userId, reportType }, 'Failed to process analytics report');
    throw error;
  }
}
 ...
 */
export const analyticsReportWorker = new Worker(
 ...
 */
export async function batchProcessAnalyticsReports(
  reportType: 'daily' | 'weekly' | 'monthly',
  format: 'csv' | 'json' | 'pdf' = 'pdf'
): Promise<{ total: number; scheduled: number; failed: number }> {
  ...
  for (const user of users) {
    try {
      ...
      logger.error({ error, userId: user.id }, 'Failed to schedule analytics report');
      failed++;
    }
  }

  logger.info({ total: users.length, scheduled, failed, reportType }, 'Analytics reports scheduled');
  return { total: users.length, scheduled, failed };
}

// Worker event handlers
analyticsReportWorker.on('completed', (job) => {
  logger.info({ jobId: job.id, userId: job.data.userId, reportType: job.data.reportType }, 'Analytics report job completed');
});

analyticsReportWorker.on('failed', (job, err) => {
  logger.error({ jobId: job?.id, error: err.message }, 'Analytics report job failed');
});
...
process.on('SIGTERM', async () => {
  ...
  logger.info('Analytics report queue shut down');
});