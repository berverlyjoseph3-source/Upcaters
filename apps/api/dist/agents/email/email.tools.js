...
class EmailTools {
    ...
    static async fetchEmails(userId, options) {
        try {
            ...
            logger_1.logger.info({ userId, options }, 'Fetching emails');
            ...
            for (const msg of (result.messages || []).slice(0, options.maxResults)) {
                try {
                    ...
                    logger_1.logger.warn({ msgError, messageId: msg.id }, 'Failed to parse individual email');
                }
            }
            return {
                emails,
            ...
            logger_1.logger.error({ error, userId }, 'Failed to fetch emails');
    ...
    static async sendEmail(userId, options) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId, subject: options.subject, to: this.formatAddresses(options.to) }, 'Sending email');
            const emailLines = [];
            // Build headers
            emailLines.push(`To: ${this.formatAddresses(options.to)}`);
            if (options.cc)
                emailLines.push(`Cc: ${this.formatAddresses(options.cc)}`);
            if (options.bcc)
                emailLines.push(`Bcc: ${this.formatAddresses(options.bcc)}`);
            if (options.replyTo)
                emailLines.push(`Reply-To: ${typeof options.replyTo === 'string' ? options.replyTo : options.replyTo.email}`);
            ...
            logger_1.logger.info({ userId, messageId: result.id, threadId: result.threadId }, 'Email sent successfully');
            return {
                id: result.id,
            ...
            logger_1.logger.error({ error, userId, subject: options.subject }, 'Failed to send email');
     ...
     * Reply to an email
     */
    static async replyToEmail(userId, emailId, body, quoteOriginal = true) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId, emailId, quoteOriginal }, 'Replying to email');
            ...
            const rawMessage = Buffer.from(emailLines.join('\r\n'))
                ...
                .replace(/\//g, '_');
            const result = await client.sendMessage(rawMessage);
            logger_1.logger.info({ userId, emailId, replyMessageId: result.id }, 'Reply sent successfully');
            return {
                id: result.id,
                threadId: result.threadId,
                success: true,
            ...
            logger_1.logger.error({ error, userId, emailId }, 'Failed to reply to email');
     ...
     * Modify email labels
     */
    static async modifyEmailLabels(userId, emailIds, addLabels, removeLabels) {
        try {
            if (emailIds.length === 0) {
                return { success: true, processedCount: 0, failedCount: 0, errors: [] };
            }
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId, emailCount: emailIds.length, addLabels, removeLabels }, 'Modifying email labels');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to modify email labels');
            return {
                success: false,
                processedCount: 0,
     ...
     */
    static async deleteEmails(userId, emailIds) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId, emailCount: emailIds.length }, 'Deleting emails');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to delete emails');
     ...
     */
    static async createDraft(userId, options) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId, subject: options.subject }, 'Creating draft');
            const emailLines = [];
            emailLines.push(`To: ${this.formatAddresses(options.to)}`);
            if (options.cc)
                emailLines.push(`Cc: ${this.formatAddresses(options.cc)}`);
            ...
            logger_1.logger.error({ error, userId }, 'Failed to create draft');
    ...
    static async getLabels(userId) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId }, 'Getting labels');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to get labels');
     ...
     */
    static async classifyEmail(userId, emailId) {
        try {
            const client = await this.getGmailClient(userId);
            ...
            logger_1.logger.info({ userId, emailId, subject }, 'Classifying email');
            ...
            try {
                ...
                try {
                    ...
                    logger_1.logger.warn({ parseError }, 'Failed to parse classification response');
                }
            }
            catch (aiError) {
                logger_1.logger.warn({ aiError }, 'AI classification failed, using heuristic fallback');
            }
            ...
            else if (lowerSubject.includes('newsletter') ||
            ...
            else if (lowerSubject.includes('win') ||
            ...
            else if (lowerSubject.includes('action required') ||
            ...
            logger_1.logger.error({ error, userId, emailId }, 'Failed to classify email');
            return {
                category: 'normal',
                priority: 5,
                suggestedAction: 'read',
     ...
     * Generate AI reply suggestion
     */
    static async generateReply(userId, emailId, tone = 'professional', length = 'medium') {
        try {
            const client = await this.getGmailClient(userId);
            const email = await client.getMessage(emailId, 'full');
            const headers = email.payload?.headers || [];
            ...
            const from = headers.find(h => h.name === 'From')?.value || '';
            const snippet = email.snippet || '';
            logger_1.logger.info({ userId, emailId, tone, length }, 'Generating reply suggestions');
            ...
            logger_1.logger.error({ error, userId, emailId }, 'Failed to generate reply');
            return {
     ...
     * Batch process emails
     */
    static async batchProcess(userId, action, emailIds, labelIds) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId, action, emailCount: emailIds.length }, 'Batch processing emails');
            switch (action) {
                case 'archive':
            ...
            logger_1.logger.error({ error, userId, action }, 'Failed to batch process');
     ...
     * Get email summary
     */
    static async getEmailSummary(userId) {
        try {
            const client = await this.getGmailClient(userId);
            logger_1.logger.info({ userId }, 'Getting email summary');
            // Get unread count
            ...
            logger_1.logger.error({ error, userId }, 'Failed to get email summary');
            return {
                totalUnread: 0,
                totalInbox: 0,
                byCategory: {},
     ...
     */
    static parseEmailAddress(addressStr) {
        if (!addressStr)
            return { email: '' };
     ...
     */
    static parseEmailAddresses(addressesStr) {
        if (!addressesStr)
            return [];
