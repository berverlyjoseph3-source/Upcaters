...
class CalendarTools {
     ...
     * List calendar events
     */
    static async listEvents(userId, options) {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId, options }, 'Listing calendar events');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to list events');
            throw error;
        }
    }
    /**
     * Create calendar event
     */
    static async createEvent(userId, options) {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId, title: options.title, start: options.start }, 'Creating calendar event');
            const eventData = {
            ...
            const result = await client.createEvent(options.calendarId || 'primary', eventData, options.sendUpdates || 'all');
            return {
                id: result.id,
                calendarId: options.calendarId || 'primary',
                ...
                visibility: result.visibility || 'default',
            ...
            logger_1.logger.error({ error, userId, title: options.title }, 'Failed to create event');
            throw error;
        }
    }
    /**
     * Update calendar event
     */
    static async updateEvent(userId, options) {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId, eventId: options.eventId }, 'Updating calendar event');
            const updates = {};
            if (options.title)
                updates.summary = options.title;
            ...
            const result = await client.patchEvent(options.calendarId || 'primary', options.eventId, updates, options.sendUpdates);
            return {
                id: result.id,
                calendarId: options.calendarId || 'primary',
                title: result.summary || '',
            ...
            logger_1.logger.error({ error, userId, eventId: options.eventId }, 'Failed to update event');
            throw error;
        }
    }
    /**
     * Delete calendar event
     */
    static async deleteEvent(userId, eventId, sendUpdates = 'all') {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId, eventId }, 'Deleting calendar event');
            await client.deleteEvent('primary', eventId, sendUpdates);
        }
        catch (error) {
            logger_1.logger.error({ error, userId, eventId }, 'Failed to delete event');
            throw error;
        }
    }
    /**
     ...
     */
    static async getFreeBusy(userId, request) {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId, timeMin: request.timeMin, timeMax: request.timeMax }, 'Getting free/busy');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to get free/busy');
            throw error;
        }
    }
    /**
     ...
     */
    static async suggestMeetingTime(userId, options) {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId, title: options.title, attendees: options.attendees }, 'Suggesting meeting times');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to suggest meeting times');
            throw error;
        }
    }
    /**
     ...
     */
    static async smartSchedule(userId, options) {
        try {
            ...
            logger_1.logger.info({ userId, eventId: event.id, start: bestSlot.start }, 'Meeting scheduled via smart scheduling');
            return event;
        }
        catch (error) {
    ...
    static async listCalendars(userId) {
        try {
            const client = await this.getCalendarClient(userId);
            logger_1.logger.info({ userId }, 'Listing calendars');
            ...
            logger_1.logger.error({ error, userId }, 'Failed to list calendars');
            throw error;
        }
    }
}
