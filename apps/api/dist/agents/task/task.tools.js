...
class TaskTools {
    ...
    static createTaskTool() {
        return {
            ...
            execute: async (params, context) => {
                ...
                return await this.createTask(context.userId, params.title, params.description, params.dueDate, params.priority, params.provider || 'google_tasks', params.projectId, params.assignee, params.labels);
            },
    ...
    static listTasksTool() {
        return {
            ...
            execute: async (params, context) => {
                return await this.listTasks(context.userId, params.status, params.provider, params.limit || 20, params.projectId, params.priority);
            },
            requiresApiCall: true,
            cost: 1,
        };
    ...
    static updateTaskTool() {
        return {
            ...
            execute: async (params, context) => {
                ...
                return await this.updateTask(context.userId, params.taskId, params.title, params.description, params.dueDate, params.priority, params.completed, params.provider, params.assignee);
            },
            requiresApiCall: true,
            cost: 1,
        };
     ...
     */
    static async getGoogleTasksClient(userId) {
        try {
            ...
            if (!token) {
                logger_1.logger.warn({ userId }, 'Google Tasks not connected');
                return null;
            }
     ...
     */
    static async getAsanaClient(userId) {
        try {
            ...
            if (!asanaToken) {
                logger_1.logger.debug({ userId }, 'Asana not connected');
                return null;
            }
     ...
     */
    static async getMondayClient(userId) {
        try {
            const user = await client_2.prisma.user.findUnique({
                where: { id: userId },
                select: { metadata: true },
            ...
            if (!mondayToken) {
                logger_1.logger.debug({ userId }, 'Monday.com not connected');
                return null;
            }
     ...
     * Create a task
     */
    static async createTask(userId, title, description, dueDate, priority, provider = 'google_tasks', projectId, assignee, labels) {
        let errors = [];
        try {
            switch (provider) {
                case 'google_tasks': {
                    ...
                    logger_1.logger.info({ userId, taskId: result.id, title }, 'Task created in Google Tasks');
                    return {
                        success: true,
                        id: result.id,
                        title: result.title,
                ...
                case 'asana': {
                    ...
                    logger_1.logger.info({ userId, taskId: result.gid, title }, 'Task created in Asana');
                    return {
                        success: true,
                        id: result.gid,
                        title: result.name,
                ...
                case 'monday': {
                    ...
                    const columnValues = {};
                    if (description)
                        columnValues.text = description;
                    if (dueDate)
                        columnValues.date = dueDate;
                    if (priority)
                        columnValues.status = priority;
                    const result = await client.createItem({
                        boardId: targetBoardId,
                        itemName: title,
                        columnValues,
                    });
                    logger_1.logger.info({ userId, taskId: result.id, title }, 'Task created in Monday.com');
                    return {
                        success: true,
                        id: result.id,
                        name: result.name,
            ...
            logger_1.logger.error({ error, userId, title, provider }, 'Failed to create task');
            return {
                success: false,
                error: errorMessage,
                provider,
     ...
     * List tasks
     */
    static async listTasks(userId, status, provider, limit = 20, projectId, priority) {
        const tasks = [];
        let errors = [];
        ...
        try {
            ...
            if (!provider || provider === 'google_tasks') {
                ...
                if (client) {
                    ...
                    try {
                        ...
                        logger_1.logger.error({ error, userId }, 'Failed to fetch Google Tasks');
                    }
                }
            }
            // Asana
            if (!provider || provider === 'asana') {
                ...
                if (client) {
                    ...
                    try {
                        ...
                        logger_1.logger.error({ error, userId }, 'Failed to fetch Asana tasks');
                    }
                }
            }
            // Monday.com
            if (!provider || provider === 'monday') {
                ...
                if (client) {
                    ...
                    try {
                        ...
                        logger_1.logger.error({ error, userId }, 'Failed to fetch Monday.com items');
                    }
                }
            }
            return {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error({ error, userId }, 'Failed to list tasks');
            return {
                success: false,
                error: errorMessage,
                tasks: [],
     ...
     * Update a task
     */
    static async updateTask(userId, taskId, title, description, dueDate, priority, completed, provider, assignee) {
        try {
            ...
            for (const prov of providers) {
                try {
                    switch (prov) {
                        case 'google_tasks': {
                            ...
                            if (completed !== undefined)
                                updates.status = completed ? 'completed' : 'needsAction';
                            const result = await client.updateTask(taskList.id, taskId, updates);
                            logger_1.logger.info({ userId, taskId, provider: prov }, 'Task updated');
                            return {
                                success: true,
                                id: result.id,
                                title: result.title,
                        ...
                        case 'asana': {
                            ...
                            if (description)
                                updates.notes = description;
                            if (dueDate)
                                updates.due_on = dueDate?.split('T')[0];
                            if (completed !== undefined)
                    ...
                    logger_1.logger.warn({ error, userId, taskId, provider: prov }, 'Update attempt failed');
                }
            }
            return {
                success: false,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error({ error, userId, taskId }, 'Failed to update task');
            return {
                success: false,
                error: errorMessage,
                taskId,
     ...
     * Delete a task
     */
    static async deleteTask(userId, taskId, provider) {
        try {
            const providers = provider ? [provider] : ['google_tasks', 'asana', 'monday'];
            let lastError = null;
            for (const prov of providers) {
                try {
                    switch (prov) {
                        case 'google_tasks': {
                            ...
                            logger_1.logger.info({ userId, taskId, provider: prov }, 'Task deleted');
                            return { success: true };
                        }
                        case 'asana': {
                            const client = await this.getAsanaClient(userId);
                    ...
                    logger_1.logger.warn({ error, userId, taskId, provider: prov }, 'Delete attempt failed');
                }
            }
            return {
                success: false,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error({ error, userId, taskId }, 'Failed to delete task');
            return {
                success: false,
                error: errorMessage,
            };
     ...
     * Complete a task
     */
    static async completeTask(userId, taskId, provider) {
        try {
            return await this.updateTask(userId, taskId, undefined, undefined, undefined, undefined, true, provider);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error({ error, userId, taskId }, 'Failed to complete task');
            return {
                success: false,
                error: errorMessage,
            };
    ...
    static async getTaskSummary(userId) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error({ error, userId }, 'Failed to get task summary');
            return {
                success: false,
                error: errorMessage,
                summary: null,
     ...
     * Get projects
     */
    static async getProjects(userId, provider) {
        const projects = [];
        const errors = [];
        try {
            // Asana projects
            if (!provider || provider === 'asana') {
                ...
                if (client) {
                    try {
                        ...
                        errors.push({ provider: 'asana', error: errorMsg });
                        logger_1.logger.error({ error, userId }, 'Failed to fetch Asana projects');
                    }
                }
            }
            // Monday.com boards
            if (!provider || provider === 'monday') {
                ...
                if (client) {
                    try {
                        ...
                        errors.push({ provider: 'monday', error: errorMsg });
                        logger_1.logger.error({ error, userId }, 'Failed to fetch Monday.com boards');
                    }
                }
            }
            // Google Tasks lists
            if (!provider || provider === 'google_tasks') {
                ...
                if (client) {
                    try {
                        ...
                        errors.push({ provider: 'google_tasks', error: errorMsg });
                        logger_1.logger.error({ error, userId }, 'Failed to fetch Google Task lists');
                    }
                }
            }
            return {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error({ error, userId }, 'Failed to get projects');
            return {
                success: false,
                error: errorMessage,
                projects: [],
     ...
     * Batch create tasks
     */
    static async batchCreateTasks(userId, tasks, provider = 'google_tasks') {
        const results = [];
        let successful = 0;
        let failed = 0;
        for (const task of tasks) {
            try {
                ...
                const result = await this.createTask(userId, task.title, task.description, task.dueDate, task.priority, provider, task.projectId);
                results.push(result);
                if (result.success) {
                    successful++;
                }
