...
class LocalStrategy {
     ...
     */
    static async validateCredentials(email, password, ipAddress, userAgent) {
        const normalizedEmail = email.toLowerCase().trim();
        // Check global rate limit (DDoS protection)
        const globalCheck = this.checkGlobalRateLimit();
        if (!globalCheck.allowed) {
            logger_1.logger.warn({ email: normalizedEmail, ip: ipAddress }, 'Global rate limit reached');
            return {
                valid: false,
        ...
        const accountRateCheck = await this.checkRateLimit(`account:${normalizedEmail}`, BRUTE_FORCE_CONFIG.maxAttempts);
        if (!accountRateCheck.allowed) {
            const blockedUntil = accountRateCheck.blockedUntil;
            logger_1.logger.warn({ email: normalizedEmail, ip: ipAddress, blockedUntil }, 'Account blocked');
            return {
                valid: false,
        ...
        let ipRateCheck = { allowed: true, remainingAttempts: Infinity, blockedUntil: null };
        if (BRUTE_FORCE_CONFIG.ipBasedBlocking && ipAddress) {
            ipRateCheck = await this.checkRateLimit(`ip:${ipAddress}`, BRUTE_FORCE_CONFIG.maxAttempts * 3);
            if (!ipRateCheck.allowed) {
                logger_1.logger.warn({ email: normalizedEmail, ip: ipAddress }, 'IP blocked');
                return {
                    valid: false,
        ...
        if (progressiveDelay > 0) {
            logger_1.logger.debug({ email: normalizedEmail, delay: progressiveDelay }, 'Applying progressive delay');
        ...
        if (!user) {
            ...
            await bcrypt_1.default.compare(password, '$2b$12$invalidhashforconstanttimecomparison');
            await this.recordFailedAttempt(`account:${normalizedEmail}`, ipAddress);
            if (ipAddress)
                await this.recordFailedAttempt(`ip:${ipAddress}`, ipAddress);
        ...
        if (!isValidPassword) {
            ...
            logger_1.logger.warn({ email: normalizedEmail, ip: ipAddress, remainingAttempts: remaining }, 'Failed login attempt');
            return {
                valid: false,
                error: 'Invalid email or password',
        ...
        logger_1.logger.info({ userId: user.id, email: user.email, ip: ipAddress }, 'User authenticated successfully');
        ...
        if (recentFailures > 0) {
            logger_1.logger.warn({ userId: user.id, recentFailures }, 'Successful login after failed attempts');
     ...
     */
    static async checkRateLimit(identifier, maxAttempts) {
        const now = Date.now();
        ...
        if (attempts.count >= maxAttempts) {
            ...
            const blockDuration = Math.min(BRUTE_FORCE_CONFIG.blockDurationMs * Math.pow(2, attempts.blockCount), BRUTE_FORCE_CONFIG.maxBlockDurationMs);
            ...
            logger_1.logger.warn({ identifier, blockedUntil, blockCount: attempts.blockCount }, 'Rate limit reached');
     ...
     */
    static calculateProgressiveDelay(identifier) {
        if (!BRUTE_FORCE_CONFIG.progressiveDelayEnabled)
            return 0;
        const attempts = loginAttemptsByAccount.get(identifier);
        if (!attempts || attempts.count === 0)
            return 0;
        return Math.min(BRUTE_FORCE_CONFIG.progressiveDelayBaseMs * Math.pow(BRUTE_FORCE_CONFIG.progressiveDelayMultiplier, attempts.count - 1), 30000 // Max 30 second delay
        );
    }
    /**
     * Record a failed login attempt
     */
    static async recordFailedAttempt(identifier, ipAddress) {
        const now = Date.now();
        ...
        VALUES (${crypto_1.default.randomBytes(16).toString('hex')}, ${identifier}, ${ipAddress || 'unknown'}, NOW(), false)
      `;
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Failed to log login attempt');
        }
    }
    /**
     ...
     */
    static async getRemainingAttempts(identifier) {
        const attempts = loginAttemptsByAccount.get(identifier);
        if (!attempts)
     ...
     */
    static async getRecentFailures(identifier) {
        const attempts = loginAttemptsByAccount.get(identifier);
        if (!attempts)
            return 0;
        const now = Date.now();
    ...
    static validatePasswordStrength(password) {
        const errors = [];
        if (password.length < auth_config_1.authConfig.password.minLength) {
            errors.push(`Password must be at least ${auth_config_1.authConfig.password.minLength} characters`);
        }
        if (auth_config_1.authConfig.password.requireUppercase && !/[A-Z]/.test(password)) {
            errors.push('Password must contain at least one uppercase letter');
        }
        if (auth_config_1.authConfig.password.requireLowercase && !/[a-z]/.test(password)) {
            errors.push('Password must contain at least one lowercase letter');
        }
        if (auth_config_1.authConfig.password.requireNumbers && !/[0-9]/.test(password)) {
            errors.push('Password must contain at least one number');
        }
        if (auth_config_1.authConfig.password.requireSpecialChars && !/[^A-Za-z0-9]/.test(password)) {
    ...
    static async cleanupOldAttempts() {
      ...
      `;
            if (result > 0) {
                logger_1.logger.debug({ deletedCount: result }, 'Old login attempts cleaned up');
            }
            ...
            logger_1.logger.error({ error }, 'Failed to cleanup old login attempts');
            return 0;
        }
    }
}
