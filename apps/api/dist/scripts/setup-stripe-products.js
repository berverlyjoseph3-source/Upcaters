...
async function setupStripeProducts() {
    ...
    for (const [planId, planConfig] of Object.entries(PLANS)) {
        ...
        const existingProducts = {
            data: activeProducts.data.filter((p) => p.metadata?.plan_type === planId.toLowerCase()),
        };
        let productId;
    ...
    for (const meteredPrice of METERED_PRICES) {
        ...
        const existingMeteredPrices = {
            data: activeMeteredPrices.data.filter((p) => p.metadata?.usage_type === meteredPrice.metadata.usage_type),
        };
        let priceId;
    ...
    const coupons = [
        { percent_off: 50, duration: 'once', name: 'LAUNCH50 - 50% off first month' },
        { percent_off: 20, duration: 'forever', name: 'ANNUAL20 - 20% off forever (annual billing)', max_redemptions: 1000 },
        { amount_off: 1000, duration: 'once', currency: 'usd', name: 'SAVE10 - $10 off' },
    ];
...
//# sourceMappingURL=setup-stripe-products.js.map