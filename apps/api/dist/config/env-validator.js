...
const CoreSchema = zod_1.z.object({
    ...
    BUILD_NUMBER: zod_1.z.string().optional().default('1'),
});
// Database Schema - Redis now REQUIRED in production
const DatabaseSchema = zod_1.z.object({
    DATABASE_URL: zod_1.z.string().url('DATABASE_URL must be a valid URL').min(1),
    DATABASE_POOL_MIN: zod_1.z.coerce.number().int().min(1).max(50).default(2),
    DATABASE_POOL_MAX: zod_1.z.coerce.number().int().min(1).max(100).default(20),
    DATABASE_IDLE_TIMEOUT: zod_1.z.coerce.number().int().min(1000).max(60000).default(10000),
    DATABASE_CONNECTION_TIMEOUT: zod_1.z.coerce.number().int().min(1000).max(30000).default(5000),
    // Redis - Enhanced validation
    REDIS_URL: zod_1.z.string().optional(),
    REDIS_PASSWORD: zod_1.z.string().optional(),
    REDIS_TLS: zod_1.z.coerce.boolean().default(false),
    REDIS_KEY_PREFIX: zod_1.z.string().default('aiagent:'),
    REDIS_MAX_RETRIES: zod_1.z.coerce.number().int().min(1).max(10).default(3),
    REDIS_RETRY_DELAY_MS: zod_1.z.coerce.number().int().min(100).max(5000).default(1000),
    // MongoDB - For vector storage
    MONGODB_URL: zod_1.z.string().optional(),
    MONGODB_REPLICA_SET: zod_1.z.string().optional().default('rs0'),
    MONGODB_SSL: zod_1.z.coerce.boolean().default(false),
});
...
const JwtSchema = zod_1.z.object({
    ...
    RATE_LIMIT_MAX_REQUESTS: zod_1.z.coerce.number().int().min(1).default(100),
    RATE_LIMIT_AUTHENTICATED_MAX: zod_1.z.coerce.number().int().min(1).default(1000),
    RATE_LIMIT_API_KEY_MAX: zod_1.z.coerce.number().int().min(1).default(2000),
    // Login security
    MAX_LOGIN_ATTEMPTS: zod_1.z.coerce.number().int().min(3).max(20).default(5),
    LOGIN_LOCKOUT_MINUTES: zod_1.z.coerce.number().int().min(5).max(480).default(30),
    BCRYPT_ROUNDS: zod_1.z.coerce.number().int().min(10).max(14).default(12),
    // Session
    SESSION_TIMEOUT_MINUTES: zod_1.z.coerce.number().int().min(5).max(480).default(30),
    MAX_CONCURRENT_SESSIONS: zod_1.z.coerce.number().int().min(1).max(20).default(5),
});
// AI Providers Schema - Enhanced with fallback chain
const AISchemaBase = zod_1.z.object({
    // OpenAI (Primary)
    OPENAI_API_KEY: zod_1.z.string().optional(),
    OPENAI_ORG_ID: zod_1.z.string().optional(),
    OPENAI_TIMEOUT: zod_1.z.coerce.number().int().min(5000).max(120000).default(30000),
    OPENAI_MAX_RETRIES: zod_1.z.coerce.number().int().min(1).max(10).default(3),
    // Anthropic (Fallback 1)
    ANTHROPIC_API_KEY: zod_1.z.string().optional(),
    ANTHROPIC_TIMEOUT: zod_1.z.coerce.number().int().min(5000).max(120000).default(60000),
    ANTHROPIC_MAX_RETRIES: zod_1.z.coerce.number().int().min(1).max(10).default(3),
    // Google Gemini (Fallback 2)
    GOOGLE_AI_API_KEY: zod_1.z.string().optional(),
    GOOGLE_AI_TIMEOUT: zod_1.z.coerce.number().int().min(5000).max(120000).default(30000),
    GOOGLE_AI_MAX_RETRIES: zod_1.z.coerce.number().int().min(1).max(10).default(3),
    // AI Provider Fallback Chain (comma-separated: openai,anthropic,google)
    AI_PROVIDER_FALLBACK_CHAIN: zod_1.z.string().default('openai,anthropic,google'),
    DEFAULT_AI_MODEL: zod_1.z.string().default('gpt-4-turbo-preview'),
    // Embedding Configuration
    EMBEDDING_MODEL: zod_1.z.string().default('text-embedding-3-small'),
    EMBEDDING_CACHE_TTL: zod_1.z.coerce.number().int().min(60).max(86400).default(3600),
    EMBEDDING_BATCH_SIZE: zod_1.z.coerce.number().int().min(1).max(100).default(10),
    // Model Fallback Configuration
    MODEL_FALLBACK_ENABLED: zod_1.z.coerce.boolean().default(true),
    MODEL_FALLBACK_CHAIN: zod_1.z.string().default('openai,anthropic,gemini'),
});
...
const StripeSchema = zod_1.z.object({
    ...
    STRIPE_API_VERSION: zod_1.z.string().default('2024-12-18.acacia'),
    // Updated Pricing Products (2025 pricing)
    STRIPE_STARTER_MONTHLY_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_STARTER_YEARLY_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_PROFESSIONAL_YEARLY_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_ENTERPRISE_MONTHLY_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_ENTERPRISE_YEARLY_PRICE_ID: zod_1.z.string().optional(),
    // Overage Prices
    STRIPE_AI_ACTIONS_OVERAGE_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_API_CALLS_OVERAGE_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_IMAGE_OVERAGE_PRICE_ID: zod_1.z.string().optional(),
    STRIPE_VIDEO_OVERAGE_PRICE_ID: zod_1.z.string().optional(),
    // Billing Configuration
    STRIPE_USAGE_REPORTING_ENABLED: zod_1.z.coerce.boolean().default(true),
    STRIPE_USAGE_REPORTING_INTERVAL: zod_1.z.coerce.number().int().min(3600).default(86400),
    BILLING_RESET_CRON: zod_1.z.string().default('0 0 1 * *'),
    WEEKLY_DIGEST_CRON: zod_1.z.string().default('0 9 * * 1'),
});
// Email Schema
const EmailSchema = zod_1.z.object({
    SENDGRID_API_KEY: zod_1.z.string().optional(),
    FROM_EMAIL: zod_1.z.string().email('FROM_EMAIL must be a valid email').optional(),
    FROM_NAME: zod_1.z.string().default('AI Agent Platform'),
    SMTP_HOST: zod_1.z.string().optional(),
    SMTP_PORT: zod_1.z.coerce.number().int().optional(),
    SMTP_USER: zod_1.z.string().optional(),
    SMTP_PASS: zod_1.z.string().optional(),
    SMTP_SECURE: zod_1.z.coerce.boolean().default(false),
    // Email notifications
    USAGE_WARNING_EMAIL_ENABLED: zod_1.z.coerce.boolean().default(true),
    USAGE_CRITICAL_EMAIL_ENABLED: zod_1.z.coerce.boolean().default(true),
    WEEKLY_DIGEST_ENABLED: zod_1.z.coerce.boolean().default(true),
});
// Web Search APIs
const WebSearchSchema = zod_1.z.object({
    ...
    PERPLEXITY_API_KEY: zod_1.z.string().optional(),
});
// Task Management APIs
const TaskManagementSchema = zod_1.z.object({
    ASANA_ACCESS_TOKEN: zod_1.z.string().optional(),
    MONDAY_API_KEY: zod_1.z.string().optional(),
});
// Monitoring Schema
...
const OrchestratorSchema = zod_1.z.object({
    // Execution Control
    MAX_CONCURRENT_EXECUTIONS: zod_1.z.coerce.number().int().min(1).max(100).default(10),
    EXECUTION_TIMEOUT_MS: zod_1.z.coerce.number().int().min(30000).max(600000).default(300000),
    DEFAULT_STEP_TIMEOUT_MS: zod_1.z.coerce.number().int().min(10000).max(120000).default(30000),
    MAX_STEPS_PER_PLAN: zod_1.z.coerce.number().int().min(1).max(50).default(10),
    MAX_RETRIES_PER_STEP: zod_1.z.coerce.number().int().min(1).max(10).default(3),
    MAX_PLAN_RETRIES: zod_1.z.coerce.number().int().min(1).max(5).default(2),
    // Retry Configuration
    RETRY_BASE_DELAY_MS: zod_1.z.coerce.number().int().min(100).max(10000).default(1000),
    RETRY_MAX_DELAY_MS: zod_1.z.coerce.number().int().min(1000).max(60000).default(30000),
    RETRY_MULTIPLIER: zod_1.z.coerce.number().min(1.5).max(4).default(2),
    // Circuit Breaker
    ENABLE_CIRCUIT_BREAKER: zod_1.z.coerce.boolean().default(true),
    CIRCUIT_BREAKER_THRESHOLD: zod_1.z.coerce.number().int().min(2).max(20).default(5),
    CIRCUIT_BREAKER_TIMEOUT_MS: zod_1.z.coerce.number().int().min(10000).max(300000).default(60000),
    // Backpressure
    ENABLE_BACKPRESSURE: zod_1.z.coerce.boolean().default(true),
    MAX_PENDING_EXECUTIONS: zod_1.z.coerce.number().int().min(10).max(1000).default(100),
    // Memory Management
    ENABLE_MEMORY_CONSOLIDATION: zod_1.z.coerce.boolean().default(true),
    ENABLE_VECTOR_SEARCH: zod_1.z.coerce.boolean().default(true),
    MAX_SHORT_TERM_MEMORIES: zod_1.z.coerce.number().int().min(10).max(200).default(50),
    SHORT_TERM_TTL_SECONDS: zod_1.z.coerce.number().int().min(300).max(86400).default(3600),
    LONG_TERM_IMPORTANCE_THRESHOLD: zod_1.z.coerce.number().min(0.1).max(1).default(0.7),
    // Pre-Execution Checks
    ENABLE_PRE_EXECUTION_COST_CHECK: zod_1.z.coerce.boolean().default(true),
    ENABLE_PLAN_OPTIMIZATION: zod_1.z.coerce.boolean().default(true),
    ENABLE_AUTOMATIC_FALLBACKS: zod_1.z.coerce.boolean().default(true),
    // Execution Persistence
    ENABLE_EXECUTION_PERSISTENCE: zod_1.z.coerce.boolean().default(true),
    PERSISTENCE_CLEANUP_DAYS: zod_1.z.coerce.number().int().min(1).max(90).default(7),
    // Streaming
    ENABLE_STREAMING: zod_1.z.coerce.boolean().default(true),
    STREAMING_CHUNK_DELAY_MS: zod_1.z.coerce.number().int().min(10).max(500).default(50),
    MAX_CHUNK_SIZE: zod_1.z.coerce.number().int().min(50).max(500).default(200),
    // Reflection
    ENABLE_EXECUTION_REFLECTION: zod_1.z.coerce.boolean().default(true),
    REFLECTION_MODEL: zod_1.z.string().default('gpt-4-turbo-preview'),
    // Rate Limiting (Orchestrator Level)
    ORCHESTRATOR_REQUESTS_PER_MINUTE: zod_1.z.coerce.number().int().min(10).max(1000).default(60),
    ORCHESTRATOR_TOKENS_PER_MINUTE: zod_1.z.coerce.number().int().min(1000).max(500000).default(90000),
    ORCHESTRATOR_COST_PER_HOUR: zod_1.z.coerce.number().min(1).max(1000).default(50),
});
// NEW: Intent Classifier Configuration Schema
const IntentClassifierSchema = zod_1.z.object({
    ENABLE_INTENT_LEARNING: zod_1.z.coerce.boolean().default(true),
    INTENT_CONFIDENCE_THRESHOLD: zod_1.z.coerce.number().min(0.1).max(1).default(0.6),
    INTENT_MAX_ALTERNATIVES: zod_1.z.coerce.number().int().min(1).max(10).default(3),
    INTENT_CACHE_TTL: zod_1.z.coerce.number().int().min(60).max(86400).default(3600),
    INTENT_CACHE_ENABLED: zod_1.z.coerce.boolean().default(true),
    INTENT_CLASSIFICATION_METHOD: zod_1.z.enum(['keyword', 'ai', 'hybrid', 'auto']).default('auto'),
    INTENT_AMBIGUITY_DETECTION: zod_1.z.coerce.boolean().default(true),
    INTENT_MULTI_INTENT_DETECTION: zod_1.z.coerce.boolean().default(true),
    MAX_CORRECTION_HISTORY: zod_1.z.coerce.number().int().min(100).max(10000).default(1000),
    CORRECTION_LEARNING_ENABLED: zod_1.z.coerce.boolean().default(true),
    CORRECTION_SIMILARITY_THRESHOLD: zod_1.z.coerce.number().min(0.5).max(1).default(0.7),
});
// Feature Flags Schema - Enhanced
const FeatureFlagsSchema = zod_1.z.object({
    // Core Features
    ENABLE_REGISTRATION: zod_1.z.coerce.boolean().default(true),
    ENABLE_MAINTENANCE_MODE: zod_1.z.coerce.boolean().default(false),
    MAINTENANCE_MESSAGE: zod_1.z.string().optional(),
    // Agent Features
    ENABLE_VIDEO_GENERATION: zod_1.z.coerce.boolean().default(false),
    ENABLE_WHITE_LABEL: zod_1.z.coerce.boolean().default(false),
    ENABLE_CUSTOM_INTEGRATIONS: zod_1.z.coerce.boolean().default(false),
    // AI Features
    ENABLE_AI_CONTENT_MODERATION: zod_1.z.coerce.boolean().default(true),
    ENABLE_AI_IMAGE_GENERATION: zod_1.z.coerce.boolean().default(true),
    ENABLE_AI_VIDEO_GENERATION: zod_1.z.coerce.boolean().default(false),
    // Security Features
    ENABLE_EMAIL_VERIFICATION: zod_1.z.coerce.boolean().default(false),
    ENABLE_TWO_FACTOR: zod_1.z.coerce.boolean().default(false),
    ENABLE_API_RATE_LIMITING: zod_1.z.coerce.boolean().default(true),
    ENABLE_CSRF_PROTECTION: zod_1.z.coerce.boolean().default(true),
    // Orchestrator Features
    ENABLE_ORCHESTRATOR: zod_1.z.coerce.boolean().default(true),
    ENABLE_EXECUTION_LOGGING: zod_1.z.coerce.boolean().default(true),
    ENABLE_PERFORMANCE_METRICS: zod_1.z.coerce.boolean().default(true),
    // Memory Features
    ENABLE_MEMORY_DEDUPLICATION: zod_1.z.coerce.boolean().default(true),
    ENABLE_MEMORY_COMPRESSION: zod_1.z.coerce.boolean().default(true),
    ENABLE_MEMORY_RELATIONSHIPS: zod_1.z.coerce.boolean().default(true),
    // Billing Features
    ENABLE_BILLING: zod_1.z.coerce.boolean().default(true),
    ENABLE_OVERAGE_TRACKING: zod_1.z.coerce.boolean().default(true),
    ENABLE_UPGRADE_RECOMMENDATIONS: zod_1.z.coerce.boolean().default(true),
    // Analytics
    ENABLE_ANALYTICS: zod_1.z.coerce.boolean().default(true),
    ENABLE_USAGE_FORECASTING: zod_1.z.coerce.boolean().default(true),
    // CORS
    CORS_ORIGINS: zod_1.z.string().default('http://localhost:3001'),
    TRUST_PROXY: zod_1.z.coerce.boolean().default(false),
    // Debug
    ENABLE_DEBUG_MODE: zod_1.z.coerce.boolean().default(false),
    DEBUG_NAMESPACE: zod_1.z.string().default('ai-agent:*'),
    ENABLE_API_DOCS: zod_1.z.coerce.boolean().default(true),
});
// ============================================
// Plan Limits Schema
// ============================================
const PlanLimitsSchema = zod_1.z.object({
    // Free Plan
    AI_ACTIONS_LIMIT_FREE: zod_1.z.coerce.number().int().default(50),
    API_CALLS_LIMIT_FREE: zod_1.z.coerce.number().int().default(100),
    TEAM_MEMBERS_LIMIT_FREE: zod_1.z.coerce.number().int().default(1),
    STORAGE_GB_LIMIT_FREE: zod_1.z.coerce.number().default(0.1),
    // Starter Plan
    AI_ACTIONS_LIMIT_STARTER: zod_1.z.coerce.number().int().default(500),
    API_CALLS_LIMIT_STARTER: zod_1.z.coerce.number().int().default(2000),
    TEAM_MEMBERS_LIMIT_STARTER: zod_1.z.coerce.number().int().default(3),
    STORAGE_GB_LIMIT_STARTER: zod_1.z.coerce.number().int().default(1),
    // Professional Plan
    AI_ACTIONS_LIMIT_PROFESSIONAL: zod_1.z.coerce.number().int().default(2500),
    API_CALLS_LIMIT_PROFESSIONAL: zod_1.z.coerce.number().int().default(15000),
    TEAM_MEMBERS_LIMIT_PROFESSIONAL: zod_1.z.coerce.number().int().default(10),
    STORAGE_GB_LIMIT_PROFESSIONAL: zod_1.z.coerce.number().int().default(10),
    // Enterprise Plan
    AI_ACTIONS_LIMIT_ENTERPRISE: zod_1.z.coerce.number().int().default(10000),
    API_CALLS_LIMIT_ENTERPRISE: zod_1.z.coerce.number().int().default(50000),
    TEAM_MEMBERS_LIMIT_ENTERPRISE: zod_1.z.coerce.number().int().default(100),
    STORAGE_GB_LIMIT_ENTERPRISE: zod_1.z.coerce.number().int().default(100),
    // Default Plan for new users
    DEFAULT_PLAN: zod_1.z.enum(['FREE', 'STARTER', 'PROFESSIONAL', 'ENTERPRISE']).default('FREE'),
    TRIAL_DAYS: zod_1.z.coerce.number().int().min(0).max(30).default(14),
});
// ============================================
...
}).refine(data => data.OPENAI_API_KEY || data.ANTHROPIC_API_KEY || data.GOOGLE_AI_API_KEY, { message: 'At least one AI provider API key must be configured (OPENAI, ANTHROPIC, or GOOGLE)' });
// ============================================
// Production-Specific Requirements
// ============================================
const ProductionRequirements = zod_1.z.object({
    NODE_ENV: zod_1.z.literal('production'),
    // Enhanced security requirements
    JWT_SECRET: zod_1.z.string().min(64, 'JWT_SECRET must be at least 64 characters in production'),
    JWT_REFRESH_SECRET: zod_1.z.string().min(64, 'JWT_REFRESH_SECRET must be at least 64 characters in production'),
    BCRYPT_ROUNDS: zod_1.z.coerce.number().int().min(12, 'BCRYPT_ROUNDS must be at least 12 in production'),
    // Redis required in production
    REDIS_URL: zod_1.z.string().min(1, 'REDIS_URL is required in production'),
    // CORS must not include localhost
    CORS_ORIGINS: zod_1.z.string().refine(val => !val.includes('localhost'), 'CORS_ORIGINS must not include localhost in production'),
    // Trust proxy required
    TRUST_PROXY: zod_1.z.literal(true, { message: 'TRUST_PROXY must be true in production' }),
    // Logging should be minimal
    LOG_LEVEL: zod_1.z.enum(['info', 'warn', 'error']).default('info'),
    // Monitoring required
    SENTRY_DSN: zod_1.z.string().url('SENTRY_DSN is required in production').optional(),
...
// ============================================
const StagingRequirements = zod_1.z.object({
    NODE_ENV: zod_1.z.literal('staging'),
    JWT_SECRET: zod_1.z.string().min(32, 'JWT_SECRET must be at least 32 characters'),
    JWT_REFRESH_SECRET: zod_1.z.string().min(32, 'JWT_REFRESH_SECRET must be at least 32 characters'),
    REDIS_URL: zod_1.z.string().optional(),
...
class EnvironmentValidator {
     ...
     */
    static validate(forceRevalidate = false) {
        if (this.validated && !forceRevalidate && this.validationCache) {
            logger_1.logger.debug('Environment already validated, returning cached result');
        ...
        try {
            ...
            logger_1.logger.error({ error }, '❌ Environment validation failed catastrophically');
            return {
                success: false,
                errors: [
                    {
     ...
     */
    static isCriticalVariable(path) {
        const criticalVars = [
            'DATABASE_URL',
            'JWT_SECRET',
            'JWT_REFRESH_SECRET',
     ...
     */
    static generateWarnings(env, environment) {
        const warnings = [];
        // === SECURITY WARNINGS ===
        if (environment === 'production') {
     ...
     */
    static checkOptionalVariables(env) {
        const missing = [];
        const optionalVars = [
            'SENTRY_DSN',
            'DATADOG_API_KEY',
     ...
     */
    static generateRecommendations(env, environment) {
     ...
     */
    static buildEnvironmentSummary(env) {
        const aiProviders = [];
        if (env.OPENAI_API_KEY)
            aiProviders.push('openai');
        if (env.ANTHROPIC_API_KEY)
...
EnvironmentValidator.validated = false;
EnvironmentValidator.validationCache = null;
// Export singleton
exports.envValidator = EnvironmentValidator;
