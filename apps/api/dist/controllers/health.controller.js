...
class HealthController {
    ...
    static async health(req, res) {
        ...
        res.json({
            ...
            services: {
                ...
                redis: redisHealth,
                websocket: {
                    connected: websocket_service_1.webSocketService.getConnectedClientsCount() > 0,
                    clients: websocket_service_1.webSocketService.getConnectedClientsCount(),
                },
                agents: {
                    total: agentsTotal,
    ...
    static async ready(req, res) {
        ...
        try {
            redisReady = redis_init_service_1.RedisInitService.isReady();
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Redis readiness check failed');
        }
    ...
    static async metrics(req, res) {
      ...
      `;
        ...
        const recentErrors = await client_1.prisma.agentExecution.count({
            where: {
        ...
        res.json({
            ...
            services: {
                ...
                websocket: {
                    clients: websocket_service_1.webSocketService.getConnectedClientsCount(),
                    subscriptions: websocket_service_1.webSocketService.getActiveSubscriptionsCount(),
                },
                agents: agentHealth,
            },
            metrics: {
     ...
     */
    static async dependencies(req, res) {
        const checks = {};
        // Check Database
    ...
    static async startup(req, res) {
        ...
        try {
            ...
            logger_1.logger.error({ error }, 'Database startup check failed');
        }
        try {
            redisReady = redis_init_service_1.RedisInitService.isReady();
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Redis startup check failed');
        }
        const allReady = dbReady && redisReady;
        if (allReady) {
