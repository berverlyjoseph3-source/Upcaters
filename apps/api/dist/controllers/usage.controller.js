...
class UsageController {
    ...
    static async getUsageStats(req, res) {
        try {
            ...
            const stats = await usage_metering_service_1.UsageMeteringService.getUsageStats(req.user.id);
            res.json({
                success: true,
                data: stats,
            });
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get usage stats');
            res.status(500).json({
                success: false,
     ...
     */
    static async getPlanLimits(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const { planId, limits, features } = await plan_gate_service_1.PlanGateService.getUserPlan(req.user.id);
            ...
            res.json({
                ...
                data: {
                    ...
                    features,
                    billingPeriod: {
                        current: usage_metering_service_1.UsageMeteringService.getCurrentBillingPeriod(),
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get plan limits');
            res.status(500).json({
                success: false,
    ...
    static async getUsageHistory(req, res) {
        try {
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get usage history');
            res.status(500).json({
                success: false,
     ...
     */
    static async getActionCosts(req, res) {
        try {
            const actions = Object.entries(usage_types_1.ACTION_COSTS).map(([key, value]) => ({
                actionType: key,
            ...
            logger_1.logger.error({ error }, 'Failed to get action costs');
            res.status(500).json({
                success: false,
     ...
     */
    static async syncUsageToDatabase(req, res) {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to sync usage');
            res.status(500).json({
                success: false,
     ...
     */
    static async resetUserUsage(req, res) {
        try {
            if (req.user?.role !== 'ADMIN') {
                res.status(403).json({
                    success: false,
            ...
            logger_1.logger.error({ error }, 'Failed to reset user usage');
            res.status(500).json({
                success: false,
     ...
     */
    static async exportUsageCSV(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to export usage CSV');
            res.status(500).json({
                success: false,
    ...
    static async checkFeatureAccess(req, res) {
        try {
            ...
            logger_1.logger.error({ error, userId: req.user?.id, feature: req.params.feature }, 'Failed to check feature access');
            res.status(500).json({
                success: false,
    ...
    static async checkUsageLimit(req, res) {
        try {
            ...
            const result = await plan_gate_service_1.PlanGateService.checkUsageLimit(req.user.id, category);
            res.json({
                success: true,
                data: result,
            });
            ...
            logger_1.logger.error({ error, userId: req.user?.id, category: req.params.category }, 'Failed to check usage limit');
            res.status(500).json({
                success: false,
     ...
     */
    static async getUsagePercentage(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const percentage = await plan_gate_service_1.PlanGateService.getUsagePercentage(req.user.id);
            res.json({
                success: true,
                data: percentage,
            });
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get usage percentage');
            res.status(500).json({
                success: false,
