...
class BillingController {
    ...
    static async getPlans(req, res) {
        try {
            const plans = Object.values(billing_types_1.PlanType)
                .filter(planId => planId !== 'CUSTOM' || req.query.includeCustom === 'true')
                .map(planId => {
                const planConfig = billing_types_1.PLANS_CONFIG[planId];
            ...
            logger_1.logger.error({ error }, 'Failed to get plans');
            res.status(500).json({
                success: false,
    ...
    static async getPlanById(req, res) {
        try {
            ...
            if (!billing_types_1.PLANS_CONFIG[planId]) {
                res.status(404).json({
                    success: false,
            ...
            logger_1.logger.error({ error }, 'Failed to get plan');
            res.status(500).json({
                success: false,
    ...
    static async createCheckoutSession(req, res) {
        try {
            ...
            const validation = billing_dto_1.CreateCheckoutSessionDtoSchema.safeParse(req.body);
            ...
            const userPlan = await plan_gate_service_1.PlanGateService.getUserPlan(req.user.id);
            ...
            res.json({
                ...
                data: {
                    ...
                    planDetails: {
                        name: planConfig.name,
                        price: interval === billing_types_1.BillingInterval.MONTHLY
                            ? planConfig.priceMonthly
                            : planConfig.priceYearly,
                        interval,
                        currency: planConfig.currency,
                        overagePricing: usage_types_1.OVERAGE_PRICING_CONFIG[planId],
                    },
                },
            });
        }
        catch (error) {
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to create checkout session');
            res.status(500).json({
                success: false,
    ...
    static async createPortalSession(req, res) {
        try {
            ...
            const validation = billing_dto_1.CreatePortalSessionDtoSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    success: false,
                    error: 'Validation failed',
            ...
            logger_1.logger.info({ userId: req.user.id }, 'Customer portal session created');
            res.json({
                success: true,
                data: {
                    url: session.url,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to create portal session');
            res.status(500).json({
                success: false,
    ...
    static async getSubscription(req, res) {
        try {
            ...
            const [subscription, overage] = await Promise.all([
                subscription_service_1.SubscriptionService.getUserSubscription(req.user.id),
                usage_metering_service_1.UsageMeteringService.getCurrentOverage(req.user.id),
            ]);
            if (!subscription) {
                const plan = await plan_gate_service_1.PlanGateService.getUserPlan(req.user.id);
                res.json({
                    success: true,
                    data: {
                        subscription: null,
                        plan: {
                            id: plan.planId,
                            name: billing_types_1.PLANS_CONFIG[plan.planId]?.name || 'Free',
                            limits: plan.limits,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get subscription');
            res.status(500).json({
                success: false,
    ...
    static async updateSubscription(req, res) {
        try {
            ...
            const validation = billing_dto_1.UpdateSubscriptionDtoSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    success: false,
                    error: 'Validation failed',
            ...
            const subscription = await subscription_service_1.SubscriptionService.updateSubscription(req.user.id, validation.data);
            res.json({
                success: true,
                data: subscription,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to update subscription');
            res.status(500).json({
                success: false,
    ...
    static async cancelSubscription(req, res) {
        try {
            ...
            const validation = billing_dto_1.CancelSubscriptionDtoSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    success: false,
                    error: 'Validation failed',
            ...
            const result = await subscription_service_1.SubscriptionService.cancelSubscription(req.user.id, validation.data.atPeriodEnd);
            logger_1.logger.info({
                userId: req.user.id,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to cancel subscription');
            res.status(500).json({
                success: false,
    ...
    static async getBillingSummary(req, res) {
        try {
            ...
            const summary = await subscription_service_1.SubscriptionService.getBillingSummary(req.user.id);
            const overage = await usage_metering_service_1.UsageMeteringService.getCurrentOverage(req.user.id);
            const plan = await plan_gate_service_1.PlanGateService.getUserPlan(req.user.id);
            const usagePercentage = await plan_gate_service_1.PlanGateService.getUsagePercentage(req.user.id);
            // Get upgrade recommendation
            let upgradeRecommendation = null;
            if (usagePercentage.aiActions.isOverLimit || usagePercentage.apiCalls.isOverLimit) {
                const optimization = await billing_analytics_service_1.BillingAnalyticsService.getPricingOptimization(req.user.id);
                if (optimization) {
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get billing summary');
            res.status(500).json({
                success: false,
     ...
     */
    static async getOverage(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const overage = await usage_metering_service_1.UsageMeteringService.getCurrentOverage(req.user.id);
            const plan = await plan_gate_service_1.PlanGateService.getUserPlan(req.user.id);
            const usage = await usage_metering_service_1.UsageMeteringService.getCurrentUsage(req.user.id);
            res.json({
                success: true,
                data: {
                    ...overage,
                    ...
                    currentUsage: usage,
                    overagePricing: plan.overagePricing,
                    resetDate: usage_metering_service_1.UsageMeteringService.getBillingPeriodEndDate(),
                    billingPeriod: usage_metering_service_1.UsageMeteringService.getCurrentBillingPeriod(),
                },
            });
        }
        catch (error) {
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get overage');
            res.status(500).json({
                success: false,
     ...
     */
    static async getUsageLimits(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const [plan, usage, percentage, overage] = await Promise.all([
                plan_gate_service_1.PlanGateService.getUserPlan(userId),
                usage_metering_service_1.UsageMeteringService.getCurrentUsage(userId),
                plan_gate_service_1.PlanGateService.getUsagePercentage(userId),
                usage_metering_service_1.UsageMeteringService.getCurrentOverage(userId),
            ]);
            res.json({
                success: true,
                data: {
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get usage limits');
            res.status(500).json({
                success: false,
     ...
     */
    static async getUpgradeRecommendation(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get upgrade recommendation');
            res.status(500).json({
                success: false,
    ...
    static async getInvoices(req, res) {
        try {
            ...
            const invoices = await invoice_service_1.InvoiceService.getUserInvoices(req.user.id, limit, offset);
            res.json({
                success: true,
                data: invoices,
            });
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get invoices');
            res.status(500).json({
                success: false,
     ...
     */
    static async getUpcomingInvoice(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const invoice = await invoice_service_1.InvoiceService.getUpcomingInvoice(req.user.id);
            const overage = await usage_metering_service_1.UsageMeteringService.getCurrentOverage(req.user.id);
            res.json({
                success: true,
                data: {
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get upcoming invoice');
            res.status(500).json({
                success: false,
    ...
    static async getInvoiceById(req, res) {
        try {
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get invoice');
            res.status(500).json({
                success: false,
     ...
     */
    static async retryInvoicePayment(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const { invoiceId } = req.params;
            const result = await invoice_service_1.InvoiceService.retryInvoicePayment(invoiceId);
            if (!result.success) {
                res.status(400).json({
                    success: false,
            ...
            logger_1.logger.info({ userId: req.user.id, invoiceId }, 'Invoice payment retry initiated');
            res.json({
                success: true,
                data: result,
            });
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to retry invoice payment');
            res.status(500).json({
                success: false,
    ...
    static async validateCoupon(req, res) {
        try {
            const validation = billing_dto_1.ValidateCouponDtoSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    success: false,
                    error: 'Validation failed',
            ...
            const couponResult = await stripe_service_1.StripeService.validateCoupon(couponCode);
            ...
            if (planId && interval) {
                const planConfig = billing_types_1.PLANS_CONFIG[planId];
                if (planConfig) {
                    const priceBeforeDiscount = interval === billing_types_1.BillingInterval.MONTHLY
                        ? planConfig.priceMonthly
                        : planConfig.priceYearly;
            ...
            logger_1.logger.error({ error }, 'Failed to validate coupon');
            res.status(500).json({
                success: false,
    ...
    static async getPaymentMethods(req, res) {
        try {
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to get payment methods');
            res.status(500).json({
                success: false,
    ...
    static async detachPaymentMethod(req, res) {
        try {
            ...
            logger_1.logger.info({ userId: req.user.id, paymentMethodId }, 'Payment method detached');
            res.json({
                success: true,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to detach payment method');
            res.status(500).json({
                success: false,
    ...
    static async setDefaultPaymentMethod(req, res) {
        try {
            ...
            await stripe_service_1.StripeService.setDefaultPaymentMethod(user.stripeCustomerId, paymentMethodId);
            res.json({
                success: true,
            ...
            logger_1.logger.error({ error, userId: req.user?.id }, 'Failed to set default payment method');
            res.status(500).json({
                success: false,
    ...
    static async getAdminReport(req, res) {
        try {
            ...
            const report = await invoice_service_1.InvoiceService.generateInvoiceReport(startDate ? new Date(startDate) : thirtyDaysAgo, endDate ? new Date(endDate) : now);
            res.json({
                success: true,
                data: report,
            });
            ...
            logger_1.logger.error({ error }, 'Failed to generate admin report');
            res.status(500).json({
                success: false,
     ...
     */
    static async getAdminOverages(req, res) {
        try {
            if (req.user?.role !== 'ADMIN') {
                res.status(403).json({
                    success: false,
            ...
            for (const user of users) {
                const overage = await usage_metering_service_1.UsageMeteringService.getCurrentOverage(user.id);
                if (overage.totalOverageCost > 0) {
                    const plan = await plan_gate_service_1.PlanGateService.getUserPlan(user.id);
            ...
            logger_1.logger.error({ error }, 'Failed to get admin overages');
            res.status(500).json({
                success: false,
                error: 'Failed to retrieve overage data',
    ...
    static async getAdminRevenue(req, res) {
        try {
            ...
            const summary = await billing_analytics_service_1.BillingAnalyticsService.getBillingMetricsSummary();
            const trends = await billing_analytics_service_1.BillingAnalyticsService.getRevenueTrend(period || 'month');
            const forecast = await billing_analytics_service_1.BillingAnalyticsService.getRevenueForecast(6);
            const planPerformance = await billing_analytics_service_1.BillingAnalyticsService.getPlanPerformance();
            const overageAnalytics = await billing_analytics_service_1.BillingAnalyticsService.getOverageAnalytics();
            res.json({
                success: true,
                data: {
                    summary,
            ...
            logger_1.logger.error({ error }, 'Failed to get admin revenue');
            res.status(500).json({
                success: false,
     ...
     */
    static async getAdminSubscriptions(req, res) {
        try {
            if (req.user?.role !== 'ADMIN') {
                res.status(403).json({
                    success: false,
            ...
            const subscriptions = await client_1.prisma.user.findMany({
                where: {
            ...
            logger_1.logger.error({ error }, 'Failed to get admin subscriptions');
            res.status(500).json({
                success: false,
    ...
    static async getAdminAnalytics(req, res) {
        try {
            ...
            switch (type) {
                case 'churn':
                    data = await billing_analytics_service_1.BillingAnalyticsService.getChurnAnalysis();
                    break;
                case 'ltv':
                    data = await billing_analytics_service_1.BillingAnalyticsService.getCustomerLifetimeValue(parseInt(req.query.limit) || 50, req.query.segment);
                    break;
                case 'overage':
                    data = await billing_analytics_service_1.BillingAnalyticsService.getOverageAnalytics(startDate ? new Date(startDate) : undefined, endDate ? new Date(endDate) : undefined);
                    break;
                case 'plans':
                    data = await billing_analytics_service_1.BillingAnalyticsService.getPlanPerformance(startDate ? new Date(startDate) : undefined, endDate ? new Date(endDate) : undefined);
                    break;
                default:
                    data = await billing_analytics_service_1.BillingAnalyticsService.getBillingMetricsSummary(startDate ? new Date(startDate) : undefined, endDate ? new Date(endDate) : undefined);
            }
            res.json({
                success: true,
                data,
            ...
            logger_1.logger.error({ error }, 'Failed to get admin analytics');
            res.status(500).json({
                success: false,
